...
public class ServicioClientes {
	...
	public Cliente buscar() {
		Cliente cliente = new Cliente("1714616132", "Santiago", 3);
	...
	@Path("buscarPorHijos/{numeroHijos}")
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	public Response buscarNumeroHijos(@PathParam("numeroHijos") int hijos) {
		ClientesBDD cli = new ClientesBDD();
		ArrayList<Cliente> clientes = null;

		try {
			clientes = cli.buscarNumeroHijos(hijos);
			return Response.ok(clientes).build();
		} catch (KrakeDevException e) {
			e.printStackTrace();
			return Response.serverError().build();
