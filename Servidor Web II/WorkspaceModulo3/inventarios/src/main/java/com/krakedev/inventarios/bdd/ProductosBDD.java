...
public class ProductosBDD {
	public ArrayList<Producto> buscar(String subcadena) throws KrakeDevException{
		...
		try {
			...
			ps=con.prepareStatement("select prod.codigo_prod, prod.nombre as nombre_producto, udm.nombre as nombre_udm, udm.descripcion as descripcion_udm,"
	...
	public void crearProducto(Producto producto) throws KrakeDevException {
		...
		try {
			con = ConexionBDD.obtenerConexion();
			PreparedStatement ps = con.prepareStatement("insert into productos(nombre, udm, precio_venta, tiene_IVA, coste, categoria, stock) "
					+ "values(?,?,?,?,?,?,?)");
			...
			e.printStackTrace();
			throw new KrakeDevException("Error al crear un nuevo producto. Detalle: "+e.getMessage());
		} catch (KrakeDevException e) {
			throw e;
		} finally {
			if (con != null) {
	...
	public void actualizar(Producto producto) throws KrakeDevException {
		...
		try {
			...
			e.printStackTrace();
			throw new KrakeDevException("Error al actualziar el producto "+producto+". Detalle: "+e.getMessage());
		} catch (KrakeDevException e) {
			throw e;
		} finally {
			if (con != null) {
